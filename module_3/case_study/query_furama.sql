...
in (select hd.ma_hop_dong from hop_dong
where month(hd.ngay_lam_hop_dong) between 10 and 12
...
having sum(ifnull(hdct.so_luong, 0)) >= (select max(hop_dong_chi_tiet.so_luong) 
...
where nv.ma_nhan_vien not in (select distinct hd.ma_nhan_vien
from hop_dong hd 
...
(select hd.ma_hop_dong from hop_dong hd where year(hd.ngay_lam_hop_dong) = 2020)
group by hdct.ma_dich_vu_di_kem
